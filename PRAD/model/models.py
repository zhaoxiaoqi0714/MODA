...
class SageLayer(nn.Module):
    ...
    def forward(self, self_feats, aggregate_feats, neighs=None):
        ...
        nodes	 -- list of nodes
        """
        aggregate_feats = torch.where(torch.isnan(aggregate_feats), torch.full_like(aggregate_feats, 0.00000001), aggregate_feats)
        if not self.gcn:
...
class GraphSage(nn.Module):
    ...
    def forward(self, nodes_batch):
        ...
        for i in range(self.num_layers):  # 每层的Sage
            lower_samp_neighs, lower_layer_nodes_dict, lower_layer_nodes = self._get_unique_neighs_list(
                nodes=lower_layer_nodes, num_sample=10)  # 获得neighbors。 聚合自己和邻居节点，点的dict，涉及到的所有节点
    ...
    def aggregate(self, nodes, pre_hidden_embs, pre_neighs, num_sample=10):
        ...
        if self.agg_func == 'MEAN':
            num_neigh = torch.sum(mask, 1, keepdim=True)  # 按行求和，保持和输入一个维度
            mask = torch.div(mask, num_neigh).to(embed_matrix.device) # 归一化操作
